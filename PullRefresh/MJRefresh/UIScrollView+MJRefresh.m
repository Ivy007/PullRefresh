 ...
 *  @param callback 回调
 */
- (void)addFooterisAutoRefresh:(BOOL)flage WithCallback:(void (^)())callback
{
    // 1.创建新的footer
    if(flage)
    {
        SKRefreshFooterView *footer = [SKRefreshFooterView footer];
 ...
 *  @param action 回调方法
 */
- (void)addFooterWithTarget:(id)target action:(SEL)action isAutoRefresh:(BOOL)flage
{
    // 1.创建新的footer
    if (!self.footer) {
        if(flage)
